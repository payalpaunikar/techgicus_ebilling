...
@Component
public class SaleItemRowExtractor implements RowExtractor<SaleItemRow>{
    @Override
    public SaleItemRow extract(Row row) {

        SaleItemRow data = new SaleItemRow();
        data.setInvoiceDate(getCellLocalDate(row.getCell(0)));
        data.setInvoiceNo(getCellString(row.getCell(1)));
